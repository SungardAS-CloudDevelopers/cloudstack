...
=======
	...
	gw = $in_gw
	dev = $in_dev
>>>>>>> 97b307d... More pseudo code in BASH scripts and enabled isRedundant in Java code. Need to design eth numbering system for device names. BTW apache web server is started in virtual router to take care of Metadata queries see: http://cloudstack.apache.org/docs/en-US/Apache_CloudStack/4.0.2/html/Admin_Guide/user-data-and-meta-data.html
	if [ "$Cflag" == "1" ]
	then
	  create_guest_network
	fi
